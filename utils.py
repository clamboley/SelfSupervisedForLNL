...
class XEntropyLoss(_WeightedLoss):
    __constants__ = ['ignore_index', 'reduction']
    ignore_index: int

    def __init__(self, weight: Optional[Tensor] = None, size_average=None, ignore_index: int = -100,
                 reduce=None, reduction: str = 'mean') -> None:
        super(XEntropyLoss, self).__init__(weight, size_average, reduce, reduction)
        self.ignore_index = ignore_index

    def forward(self, input: Tensor, target: Tensor, idx) -> Tensor:
    ...
    def __init__(self, root, train=True,
        ...
        if self.train:
            ...
            if noise_type != 'clean':
                ...
                if not is_human:
                    tr = np.zeros((self.nb_classes, self.nb_classes))
                    for i in range(len(self.train_noisy_labels)):
                        tr[self.train_labels[i]][self.train_noisy_labels[i]] += 1
                    tr = tr/np.sum(tr, axis=1)
                    print(f'Noise transition matrix is \n{tr}')
                    train_noisy_labels = multiclass_noisify(y=np.array(self.train_labels), P=tr,
    ...
    def load_label(self):
        ...
        if isinstance(noise_label, dict):
            if "clean_label" in noise_label.keys():
                ...
                print(f'The overall noise rate is'
                      f'{1-np.mean(clean_label.numpy() == noise_label[self.noise_type + "_label"])}')
            return noise_label[self.noise_type + "_label"].reshape(-1)
        else:
